/* (C)2024*/
package org.visual.designer.customskins;
...
import org.visual.designer.customskins.titled.TitledConnectorSkin;
import org.visual.designer.customskins.titled.TitledNodeSkin;
import org.visual.designer.customskins.titled.TitledSkinConstants;
import org.visual.designer.customskins.titled.TitledTailSkin;
import org.visual.designer.selections.SelectionCopier;
import org.visual.graph.editor.api.*;
import org.visual.graph.editor.core.skins.defaults.DefaultConnectorSkin;
import org.visual.graph.editor.core.skins.defaults.DefaultNodeSkin;
import org.visual.graph.editor.core.skins.defaults.DefaultTailSkin;
...
public class TitledSkinController extends DefaultSkinController {
   ...
   */
  public TitledSkinController(
      final GraphEditor graphEditor, final GraphEditorContainer graphEditorContainer) {
  ...
  private GNodeSkin createSkin(final GNode node) {
    return TitledSkinConstants.TITLED_NODE.equals(node.getType())
        ? new TitledNodeSkin(node)
        : new DefaultNodeSkin(node);
  }

  private GConnectorSkin createSkin(final GConnector connector) {
    return TitledSkinConstants.TITLED_INPUT_CONNECTOR.equals(connector.getType())
            || TitledSkinConstants.TITLED_OUTPUT_CONNECTOR.equals(connector.getType())
        ? new TitledConnectorSkin(connector)
        : new DefaultConnectorSkin(connector);
  }

  private GTailSkin createTailSkin(final GConnector connector) {
    return TitledSkinConstants.TITLED_INPUT_CONNECTOR.equals(connector.getType())
            || TitledSkinConstants.TITLED_INPUT_CONNECTOR.equals(connector.getType())
        ? new TitledTailSkin(connector)
