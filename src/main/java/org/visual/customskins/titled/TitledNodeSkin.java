/* (C)2005-2014*/
package org.visual.customskins.titled;
...
public class TitledNodeSkin extends GNodeSkin {
  ...
  private static final String STYLE_CLASS_SELECTION_HALO =
      "titled-node-selection-halo"; // $NON-NLS-1$
    ...
    if (connectorSkins != null) {
      for (final GConnectorSkin connectorSkin : connectorSkins) {
        ...
        final boolean isOutput =
            connectorSkin.getItem().getType().contains("output"); // $NON-NLS-1$
  ...
  private void layoutLeftAndRightConnectors() {
    ...
    for (int i = 0; i < inputCount; i++) {
      ...
      final double layoutY =
          GeometryUtils.moveOnPixel((i + 1) * inputOffsetY - inputSkin.getHeight() / 2);
    ...
    for (int i = 0; i < outputCount; i++) {
      ...
      final double layoutX =
          GeometryUtils.moveOnPixel(getRoot().getWidth() - outputSkin.getWidth() / 2);
      final double layoutY =
          GeometryUtils.moveOnPixel((i + 1) * outputOffsetY - outputSkin.getHeight() / 2);
