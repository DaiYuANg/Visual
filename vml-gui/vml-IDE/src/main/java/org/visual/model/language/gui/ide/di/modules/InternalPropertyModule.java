...
import lombok.val;
import org.visual.model.language.gui.ide.constants.InternalConfigKey;
...
public class InternalPropertyModule extends AbstractModule {
	...
	private void bindAboutApplication() {
		...
		bind(String.class).annotatedWith(Names.named("ApplicationName")).toInstance(applicationName);

		bind(String.class).annotatedWith(Names.named("ApplicationCache"))
				.toInstance(BaseDirectories.get().cacheDir + applicationName);

		bind(String.class).annotatedWith(Names.named("ApplicationWorkspace"))
				.toInstance(BaseDirectories.get().homeDir + File.separator + applicationName);

		bind(String.class).annotatedWith(Names.named("ApplicationData"))
