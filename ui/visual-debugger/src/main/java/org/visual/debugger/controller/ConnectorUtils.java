...
public class ConnectorUtils {
  ...
  public static int getBranchCount(final Node node) {
    ...
    int c = 1;
    final List<Node> children = NodeUtil.getChildren(node);
    c += children.stream().mapToInt(ConnectorUtils::getBranchCount).sum();
    return c;
  }
  ...
  public static String nodeClass(final @NotNull Object node) {
    ...
    if (value == null) {
      ...
      while (name.isEmpty()
          || name.contains("anon$")) { // getSimpleName for anonymous classes in Scala does not
        // return empty
        // string, instead it will contain some encoded name. Check it here.
        // Also see
        // https://github.com/scala/scala/blob/2.11.x/test/files/jvm/javaReflection/Test.scala
  ...
  public static List<Animation> getAnimations() {
    ...
    //            @SuppressWarnings("unchecked") final PulseReceiver[] object =
    // (PulseReceiver[])
    // field.get(timer);
    //            for (PulseReceiver pulseReceiver : object) {
    //                if (pulseReceiver instanceof AnimationPulseReceiver) {
    //                    final Field field2 =
    // AnimationPulseReceiver.class.getDeclaredField("animation");
