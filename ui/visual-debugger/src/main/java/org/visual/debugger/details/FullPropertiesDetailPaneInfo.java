...
class FullPropertiesDetailPaneInfo extends DetailPaneInfo {
  ...
  private void createPropertiesPanel() {
    ...
    if (node != null) {
      ...
      list.forEach(
          metaData -> {
            val wvalue = ((CssMetaData) metaData).getStyleableProperty(node);
            styles.put(wvalue, metaData.getProperty());
          });
    }
  ...
  protected void updateDetail(final String propertyName, final boolean all) {
    ...
    if (observable == null) {
      log.atInfo().log("null PropertyName for:" + propertyName);
      return;
    }
    final Object value = observable.getValue();
    switch (value) {
      case Image image -> detail.setValue("Image (" + image.getUrl() + ")");
        ...
        if (!background.getFills().isEmpty()) {
          ...
          detailString.append(
              background.getFills().stream()
                  .map(FullPropertiesDetailPaneInfo::backgroundFillToString)
        ...
        if (!background.getImages().isEmpty()) {
          ...
          detailString.append(
              background.getImages().stream()
                  .map(FullPropertiesDetailPaneInfo::backgroundImageToString)
                  .collect(Collectors.joining(",\n  ")));
          detailString.append("]");
        }
      ...
      case Border border -> {
        ...
        if (!border.getStrokes().isEmpty()) {
          ...
          detailString.append(
              border.getStrokes().stream()
                  .map(FullPropertiesDetailPaneInfo::borderStrokeToString)
                  .collect(Collectors.joining(",\n  ")));
          detailString.append("]");
        }
        if (!border.getImages().isEmpty()) {
          detailString.append("images=[\n  ");
          detailString.append(
              border.getImages().stream()
                  .map(FullPropertiesDetailPaneInfo::borderImageToString)
                  .collect(Collectors.joining(",\n  ")));
          detailString.append("]");
        }
    ...
    if (observable instanceof Property) {
      if (observable.getValue() instanceof Enum) {
        detail.setEnumProperty(
            (Property) observable, (Class<? extends Enum>) observable.getValue().getClass());
      ...
      if (observable instanceof ReadOnlyProperty) {
        ...
        log.atInfo().log("Strange Property:{}", observable);
      }
    }
    if (!all) detail.updated();
  }
  ...
  private static String borderStrokeToString(BorderStroke borderStroke) {
    return "paint="
        ...
        + "\n    widths="
        + borderWidthsToString(borderStroke.getWidths())
        + " insets="
  ...
  private static String borderImageToString(BorderImage borderImage) {
    return "image="
        ...
        + "\n    widths="
        + borderWidthsToString(borderImage.getWidths())
        + "\n    slices="
        + borderWidthsToString(borderImage.getSlices())
        + "\n    insets="
  ...
  private static String backgroundImageToString(BackgroundImage backgroundImage) {
    return "image="
        ...
        + " position="
        + backgroundPositionToString(backgroundImage.getPosition())
        + " repeatX="
        + backgroundImage.getRepeatX()
        + " repeatY="
        + backgroundImage.getRepeatY()
        + " size="
        + backgroundSizeToString(backgroundImage.getSize());
  }

  private static String backgroundSizeToString(BackgroundSize size) {
    String width =
        size.getWidth() == BackgroundSize.AUTO
            ? "AUTO"
            : size.getWidth() + (size.isWidthAsPercentage() ? "%" : "");
    String height =
        size.getHeight() == BackgroundSize.AUTO
            ? "AUTO"
  ...
  private static String backgroundPositionToString(BackgroundPosition position) {
    String horizPos =
        position.getHorizontalPosition() + (position.isHorizontalAsPercentage() ? "%" : "");
    String vertPos =
        position.getVerticalPosition() + (position.isVerticalAsPercentage() ? "%" : "");
    return position.getHorizontalSide()
        + " "
        + horizPos
        + " "
        + position.getVerticalSide()
        + " "
        + vertPos;
  }
