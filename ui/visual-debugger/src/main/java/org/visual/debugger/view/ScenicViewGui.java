...
import org.visual.debugger.event.*;
import org.visual.debugger.model.update.AppsRepository;
import org.visual.debugger.node.SVNode;
import org.visual.debugger.view.control.FilterTextField;
import org.visual.debugger.view.dialog.AboutBox;
import org.visual.debugger.view.tabs.*;

/** The base UI */
@Slf4j
public class ScenicViewGui {
  public static final String STYLESHEETS =
      Objects.requireNonNull(ScenicViewGui.class.getResource("scenicview.css")).toExternalForm();
   ...
   */
  @Getter private ScenegraphTreeView treeView;
  ...
  @Getter
  private final FXConnectorEventDispatcher stageModelListener =
      new FXConnectorEventDispatcher() {
        @Override
        public void dispatchEvent(final FXConnectorEvent appEvent) {
          if (isValid(appEvent)) {
            // doDispatchEvent(appEvent);
            switch (appEvent.getType()) {
              case ROOT_UPDATED -> doDispatchEvent(appEvent);
              case MOUSE_POSITION -> {
            ...
            log.info("Unused event " + appEvent);
          }
        }
            ...
            new KeyFrame(
                ...
                event -> {
                  ...
                  while (!eventQueue.isEmpty()) {
                    doDispatchEvent(eventQueue.removeFirst());
                  }
                }));
  ...
  private void buildUI() {
    ...
    tabPane
        ...
        .addListener((ov, oldValue, newValue) -> updateMenuBar(oldValue, newValue));
    ...
    tabPane
        .getTabs()
        .addAll(detailsTab, eventsTab, animationsTab, javadocTab, cssfxTab, threeDOMTab);
    // /3Dom
  ...
  private void buildFiltersBox() {
    ...
    propertyFilterField.setOnKeyReleased(
    ...
    activeNodeFilters.add(
        new NodeFilter() {
          ...
          @Override
          public boolean accept(final SVNode node) {
            if (idFilterField.getText().isEmpty()) return true;
            return node.getId() != null
                && node.getId().toLowerCase().contains(idFilterField.getText().toLowerCase());
          }
    ...
    activeNodeFilters.add(
        new NodeFilter() {
          ...
          @Override
          public boolean accept(final SVNode node) {
            if (classNameFilterField.getText().isEmpty()) return true;

            // Allow reduces or complete className
            return node.getNodeClass()
                .toLowerCase()
                .contains(classNameFilterField.getText().toLowerCase());
          }
          ...
          @Override
          public boolean expandAllNodes() {
            return !classNameFilterField.getText().isEmpty();
          }
        });
  }
  ...
  private void buildMenuBar() {
    ...
    // properties.getProperty(ScenicViewBooter.JDK_PATH_KEY);
    //                final File jdkPathFile = new
    // ClassPathDialog(toolsPath).show(scenicViewStage);
    //
    //                if (jdkPathFile != null) {
    //                    properties.setProperty(ScenicViewBooter.JDK_PATH_KEY,
    // jdkPathFile.getAbsolutePath());
    ...
    // ---- Options Menu
    final CheckMenuItem showBoundsCheckbox =
        buildCheckMenuItem(
            "Show Bounds Overlays",
            "Show the bound overlays on selected",
            "Do not show bound overlays on selected",
            "showBounds",
            Boolean.TRUE);
    showBoundsCheckbox.setId("show-bounds-checkbox");
    // showBoundsCheckbox.setTooltip(new
    // Tooltip("Display a yellow highlight for boundsInParent and green outline for
    // layoutBounds."));
    ...
    final CheckMenuItem collapseControls =
        buildCheckMenuItem(
            "Collapse controls In Tree",
            "Controls will be collapsed",
            "Controls will be expanded",
            "collapseControls",
            Boolean.TRUE);
    collapseControls
        .selectedProperty()
    ...
    final CheckMenuItem collapseContentControls =
        buildCheckMenuItem(
            "Collapse container controls In Tree",
            "Container controls will be collapsed",
            "Container controls will be expanded",
            "collapseContainerControls",
            Boolean.FALSE);
    ...
    final CheckMenuItem showBaselineCheckbox =
        buildCheckMenuItem(
            "Show Baseline Overlay",
            "Display a red line at the current node's baseline offset",
            "Do not show baseline overlay",
            "showBaseline",
            Boolean.FALSE);
    ...
    final CheckMenuItem automaticScenegraphStructureRefreshing =
        buildCheckMenuItem(
            "Auto-Refresh Scenegraph",
            "Scenegraph structure will be automatically updated on change",
            "Scenegraph structure will NOT be automatically updated on change",
            "automaticScenegraphStructureRefreshing",
            Boolean.TRUE);
    automaticScenegraphStructureRefreshing
        .selectedProperty()
        .addListener(
            (ChangeListener<Boolean>)
                (o, oldValue, newValue) -> {
                  configuration.setAutoRefreshSceneGraph(
                      automaticScenegraphStructureRefreshing.isSelected());
                  configurationUpdated();
                });
    ...
    showSearchBar =
        buildCheckMenuItem(
            "Show Search Bar",
            "Shows a search bar to allow you to filter the displayed information",
            "Shows a search bar to allow you to filter the displayed information",
            "showSearchBar",
            Boolean.TRUE);
    ...
    showInvisibleNodes =
        buildCheckMenuItem(
            "Show Invisible Nodes In Tree",
            "Invisible nodes will be faded in the scenegraph tree",
            "Invisible nodes will not be shown in the scenegraph tree",
            "showInvisibleNodes",
            Boolean.FALSE);
    final ChangeListener<Boolean> visilityListener =
        (o, oldValue, newValue) -> {
          configuration.setVisibilityFilteringActive(
              !showInvisibleNodes.isSelected() && !showFilteredNodesInTree.isSelected());
          configurationUpdated();
        };
    ...
    showNodesIdInTree =
        buildCheckMenuItem(
            "Show Node IDs",
            "Node IDs will be shown on the scenegraph tree",
            "Node IDs will not be shown the Scenegraph tree",
            "showNodesIdInTree",
            Boolean.FALSE);
    ...
    showFilteredNodesInTree =
        buildCheckMenuItem(
            "Show Filtered Nodes In Tree",
            "Filtered nodes will be faded in the tree",
            "Filtered nodes will not be shown in tree (unless they are parents of non-filtered nodes)",
            "showFilteredNodesInTree",
            Boolean.TRUE);
    ...
    componentSelectOnClick =
        buildCheckMenuItem(
            "Component highlight/select on click",
            "Click on the scene to select a component",
            "",
            null,
            null);
    componentSelectOnClick
        .selectedProperty()
        .addListener((ChangeListener<Boolean>) (o, oldValue, newValue) -> selectOnClick(newValue));

    final CheckMenuItem ignoreMouseTransparentNodes =
        buildCheckMenuItem(
            "Ignore MouseTransparent Nodes",
            "Transparent nodes will not be selectable",
            "Transparent nodes can be selected",
            "ignoreMouseTransparentNodes",
            Boolean.TRUE);
    ignoreMouseTransparentNodes
        .selectedProperty()
        .addListener(
    ...
    final CheckMenuItem registerShortcuts =
        buildCheckMenuItem(
            "Register shortcuts",
            "SV Keyboard shortcuts will be registered on your app",
            "SV Keyboard shortcuts will be removed on your app",
            "registerShortcuts",
            Boolean.TRUE);
    registerShortcuts
        .selectedProperty()
    ...
    autoRefreshStyleSheets =
        buildCheckMenuItem(
            "Auto-Refresh StyleSheets",
            "A background thread will check modifications on the css files to reload them if needed",
            "StyleSheets autorefreshing disabled",
            "autoRefreshStyleSheets",
            Boolean.FALSE);
    autoRefreshStyleSheets
        .selectedProperty()
    ...
    //        final CheckMenuItem showRuler = buildCheckMenuItem("Show Ruler", "Show ruler in
    // the
    // scene for alignment purposes", "", null, null);
    //        showRuler.selectedProperty().addListener((ChangeListener<Boolean>) (o, oldValue,
    // newValue) -> {
    ...
    //
    //            @Override public void changed(final ObservableValue<? extends Color> arg0,
    // final
    // Color arg1, final Color newValue) {
    //                final int red = (int) (newValue.getRed() * 255);
    //                final int green = (int) (newValue.getGreen() * 255);
    //                final int blue = (int) (newValue.getBlue() * 255);
    //                configuration.setRulerColor(toHexByte(red) + toHexByte(green) +
    // toHexByte(blue));
    //                configurationUpdated();
    //            }
    //
    //            private String toHexByte(final int value) {
    //                return (value < 16) ? "0" + Integer.toString(value, 16) :
    // Integer.toString(value, 16);
    //            }
    //        });
    //        rulerConfig.rulerSeparationProperty().addListener((ChangeListener<Number>) (o,
    // oldValue, newValue) -> {
    ...
    //        help.setOnAction(event -> HelpBox.make("Help Contents", HELP_URL,
    // scenicViewStage));
  ...
  private void updateMenuBar(final Tab oldValue, final Tab newValue) {
    if (oldValue instanceof ContextMenuContainer) {
      Menu menu = ((ContextMenuContainer) oldValue).getMenu();
      menuBar.getMenus().remove(menu);
    }
    if (newValue instanceof ContextMenuContainer) {
  ...
  //        try {
  //            if (forced || value == null || ((System.currentTimeMillis() -
  // format.parse(value).getTime()) > 86400000)) {
  ...
  //                                versionNum = newVersion.substring(0,
  // newVersion.indexOf('\n'));
  //                            }
  ...
  //
  //                        if (versionNum != null) {
  //                            new InfoBox("Version check", "New version found:" + versionNum + "
  // (Yours is:" + ScenicView.VERSION + ")", newVersion, 400, 200);
  //                        } else if (forced) {
  //                            new InfoBox("Version check", "You already have the latest version
  // of
  // Scenic View.", null, 400, 150);
  //                        }
  //
  //                        Persistence.saveProperty("lastVersionCheck", format.format(new
  // Date()));
  //                    }
  //                });
  //
        ...
        .forEach(
            stages ->
                stages.stream()
                    ...
                    .findFirst()
                    .ifPresent(StageController::updateAnimations));
  }
  ...
  private void storeSelectedNode(final SVNode value) {
    ...
    if (selectedNode != null && detailsTab.isSelected())
      setStatusText(
          "Click on the labels to modify its values. The panel could have different capabilities. When changed the values will be highlighted",
          8000);
    activeStage.setSelectedNode(value);
  }

  public CheckMenuItem buildCheckMenuItem(
      final String text,
  ...
  private FilterTextField createFilterField(
      final String prompt, final EventHandler<KeyEvent> keyHandler) {
  ...
  //        layoutInArea(borderPane, getPadding().getLeft(), getPadding().getTop(), getWidth() -
  // getPadding().getLeft() - getPadding().getRight(), getHeight()
    ...
    switch (appEvent.getType()) {
      ...
      case WINDOW_DETAILS -> {
        ...
        if (isActive(wevent.getStageID())) {
          statusBar.updateWindowDetails(
              wevent.getWindowType(), wevent.getBounds(), wevent.isFocused());
        }
      }
      case NODE_SELECTED -> {
      ...
      case ROOT_UPDATED -> {
        treeView.updateStageModel(
            getStageController(appEvent.getStageID()),
      ...
      case DETAILS -> {
        ...
        detailsTab.updateDetails(
            ev.getPaneType(),
            ev.getPaneName(),
            ev.getDetails(),
            (detail, value) -> {
              getStageController(appEvent.getStageID())
                  .setDetail(detail.getDetailType(), detail.getDetailID(), value);
            });
      }
      case DETAIL_UPDATED -> {
      ...
      case ANIMATIONS_UPDATED -> {
        animationsTab.update(
            appEvent.getStageID(), ((AnimationsCountEvent) appEvent).getAnimations());
      }
      case CSS_ADDED, CSS_REMOVED, CSS_REPLACED -> cssfxTab.handleEvent(appEvent);
      default -> {
        log.info("Unused event for type " + appEvent);
      }
    }
  }
  ...
  private int indexOfNode(final SVNode node, final boolean add) {
    for (int i = 0; i < eventQueue.size(); i++) {
      ...
      if ((!add || ev.getType() != FXConnectorEvent.SVEventType.NODE_REMOVED)
          && (add || ev.getType() != FXConnectorEvent.SVEventType.NODE_ADDED)) {
        continue;
      }
