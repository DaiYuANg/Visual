...
class RemoteConnectorImpl extends UnicastRemoteObject implements RemoteConnector, FXConnector {
  ...
  @Serial private static final long serialVersionUID = -8263538629805832734L;
  ...
  public void onAgentStarted(final int port) {
    log.info("Remote agent started on port:" + port);
    RMIUtils.findApplication(
        port,
        application -> {
  ...
  private void addStages(
      final int appsID, final StageID[] ids, final RemoteApplication application) {
    final AppControllerImpl impl =
        new AppControllerImpl(appsID, Integer.toString(appsID)) {
          @Override
          public void close() {
            super.close();
            try {
    ...
    for (int i = 0; i < ids.length; i++) {
      log.info("RemoteApp connected on:" + port + " stageID:" + ids[i]);
      final int cont = i;
      impl.getStages()
          .add(
              new StageController() {
                ...
                public void setDetail(
                    final DetailPaneType detailType, final int detailID, final String value) {
                  try {
  ...
  /**
   * This method is periodically call to connect to remote VM that may have JavaFX Application
   * running on them
   */
  @Override
  public List<AppController> connect() {
    ...
    log.info(machines.size() + " JavaFX applications found");
      ...
      for (final VirtualMachine machine : machines) {
        ...
        if (applications.containsKey(temp.id())) {
          ...
          try {
            ...
            log.error(e.getMessage(), e, "Failure connecting to machine.");
  ...
  private void loadAgent(final VirtualMachine machine, final File agentFile) {
    try {
      ...
      machine.loadAgent(
          agentFile.getAbsolutePath(),
          port + ":" + this.port + ":" + machine.id() + ":" + log.isDebugEnabled());
      machine.detach();
    } catch (final Exception e) {
      log.error(e.getMessage(), e);
    }
  ...
  private List<VirtualMachine> getRunningJavaFXApplications() {
    ...
    log.info("Number of running Java applications found: " + machines.size());
    ...
    for (final VirtualMachineDescriptor vmd : machines) {
      ...
      try {
        ...
        log.info("Obtaining properties for Java application with PID:" + virtualMachine.id());
        ...
        if (sysPropertiesMap != null
            && sysPropertiesMap.containsKey(
                JAVAFX_SYSTEM_PROPERTIES_KEY) /* && !sysPropertiesMap.containsKey(SCENIC_VIEW_VM)*/) {
          javaFXMachines.add(virtualMachine);
        } else {
          virtualMachine.detach();
        }
        //                Logger.print("JVM:" + virtualMachine.id() + " detection
        // finished");
      } catch (final AttachNotSupportedException | IOException | InternalError ex) {
        dumpAttachError(vmd, ex);
      }
    }
    //        if (debug && javaFXMachines.isEmpty() && machines.size() > 1) {
    //            debug("No running JavaFX applications found.");
    //            for (final Iterator<String> iterator = vmsProperties.keySet().iterator();
    // iterator.hasNext();) {
    ...
    //                    debug("ID:" + id);
    //                    for (@SuppressWarnings("rawtypes") final Iterator iterator2 =
    // properties.keySet().iterator(); iterator2.hasNext();) {
  ...
  private File findAgent() {
    ...
    try {
      ...
      if (Objects.requireNonNull(url).getProtocol().equals("jar")) {
      ...
      log.error(e.getMessage(), e, "Attempting to get agent jar.");
    }
    ...
    if (tempf == null || !tempf.exists()) {
      ...
      if (classPath != null && !classPath.isEmpty()) {
        ...
        for (String file : files) {
          if (file.toLowerCase().contains("scenicview.jar")) {
            tempf = new File(file);
            break;
          }
        }
    ...
    if (tempf == null || !tempf.exists()) {
      // if we are here, lets check the development location, and try to get the jar from
      // there
      ...
      if (buildLibsDir.exists()) {
        ...
        for (File jarFile : Objects.requireNonNull(jarFiles)) {
      ...
      // System.err.println("Cannot load the agent, ScenicView.jar not found here:" +
      // tempf.getAbsolutePath());
    }

    if (tempf == null) {
      log.info("Error: Unable to find agent jar file. Exiting Scenic View.");
      System.exit(-1);
    } else {
      log.info("Loading agent from: " + tempf.getAbsolutePath());
    }
    return tempf;
  }
}
