...
import org.visual.debugger.details.AllDetails;
import org.visual.debugger.details.DetailPaneType;
import org.visual.debugger.event.*;
import org.visual.debugger.gui.ComponentHighLighter;
import org.visual.debugger.gui.RuleGrid;
import org.visual.debugger.helper.StyleSheetRefresher;
import org.visual.debugger.helper.SubWindowChecker;
import org.visual.debugger.module.api.CSSFXEvent;
import org.visual.debugger.module.api.MonitoredStylesheet;
...
import org.visual.debugger.node.SVNodeFactory;
...
public class StageControllerImpl implements StageController {
  ...
  @Setter boolean remote;

  AllDetails details;

  private final EventHandler<? super MouseEvent> sceneHoverListener =
      ev -> {
  ...
  private final EventHandler<? super MouseEvent> scenePressListener =
      ev ->
          dispatchEvent(
              new NodeSelectedEvent(getID(), createNode(getHoveredNode(ev.getX(), ev.getY()))));

  /** Listeners and EventHandlers */
  private final EventHandler<? super Event> traceEventHandler =
      event -> {
        if (configuration.isEventLogEnabled()) {
          dispatchEvent(
              new EvLogEvent(
                  getID(),
                  createNode((Node) event.getSource()),
  ...
  private final EventHandler<? super MouseEvent> mousePosListener =
      ev ->
          dispatchEvent(
              new MousePosEvent(getID(), (int) ev.getSceneX() + "x" + (int) ev.getSceneY()));
  ...
  public StageControllerImpl(
    ...
    visibilityInvalidationListener =
        (o, oldValue, newValue) -> {
          if (configuration.isAutoRefreshSceneGraph()) {
            @SuppressWarnings("unchecked")
            final Node bean = (Node) ((Property<Boolean>) o).getBean();
            ...
            if (filteringActive && !newValue) {
              ...
              /** This should be improved ideally we use request a repaint for the TreeItem */
              removeNode(bean, false);
              addNewNode(bean);
            }
          }
  ...
  private void updateBoundsRects(final Node selectedNode) {
    ...
    if (selectedNode != null && selectedNode.getScene() == targetScene) {
      SCUtils.updateRect(
          overlayParent, selectedNode, selectedNode.getBoundsInParent(), 0, 0, boundsInParentRect);
      SCUtils.updateRect(
          overlayParent,
          selectedNode,
          selectedNode.getLayoutBounds(),
  ...
  public void update() {
    ...
    if (targetScene != null && targetScene.getRoot() == target) {
      ...
      val app = new SVDummyNode(title, "Stage", getID().getStageID(), NodeType.STAGE);
    ...
    dispatchEvent(new NodeAddRemoveEvent(FXConnectorEvent.SVEventType.ROOT_UPDATED, getID(), root));
    updateSceneDetails();
  }
  ...
  private void updateSceneDetails() {
    ...
    if (dispatcher != null) {
      dispatchEvent(
          new SceneDetailsEvent(
              getID(),
              nodeCount,
              targetScene != null
                  ? ConnectorUtils.format(targetScene.getWidth())
                      + " x "
                      + ConnectorUtils.format(targetScene.getHeight())
                  : ""));
    }
  ...
  private void setTargetWindow(final Window value) {
    ...
    if (targetWindow != null) {
      ...
      if (targetWindow instanceof Stage) stageID.setName(((Stage) targetWindow).getTitle());
    }
    updateWindowDetails();
  }
  ...
  private void updateWindowDetails() {
    if (dispatcher != null) {
      if (targetWindow != null) {
        dispatchEvent(
            new WindowDetailsEvent(
                getID(),
                targetWindow.getClass().getSimpleName(),
                ConnectorUtils.boundsToString(
                    targetWindow.getX(),
                    targetWindow.getY(),
                    targetWindow.getWidth(),
                    targetWindow.getHeight()),
                targetWindow.isFocused(),
                canStylesheetsBeRefreshed()));
      } else {
  ...
  private void setTarget(final Parent value) {
    ...
    if (overlayParent == null) {
      log.info("warning: could not find writable parent to add overlay nodes; overlays disabled.");
    ...
    if (previousHightLightedData != nodeData) {
      ...
      if (nodeData != null) {
        ...
        componentHighLighter =
            new ComponentHighLighter(
                createNode(nodeData),
                targetWindow != null ? targetWindow.getWidth() : -1,
                targetWindow != null ? targetWindow.getHeight() : -1,
                SCUtils.toSceneBounds(overlayParent, nodeData, nodeData.getBoundsInParent(), 0, 0));
  ...
  public void configurationUpdated(final Configuration configuration) {
    ...
    } else if (configuration.getRulerSeparation() != this.configuration.getRulerSeparation()
        && grid != null) {
    ...
    if (!configuration.getRulerColor().equals(this.configuration.getRulerColor())) {
      ...
      if (grid != null) grid.setStroke(Color.web(configuration.getRulerColor()));
    }
  ...
  public void setSelectedNode(final SVNode svNode) {
    ...
    if (svNode != null) {
      ...
      if (selectedNode != null) {
        ...
        //                // we also need to track if this node is contained within a
        // SubScene,
  ...
  private void updateBaseline() {
    if (this.configuration.isShowBaseline() && selectedNode != null) {
      ...
      updateBaseline(
          true,
          selectedNode.localToScene(bounds.getMinX(), bounds.getMinY() + baseline),
          bounds.getWidth());
  ...
  private void addNewNode(final Node node) {
    if (SCUtils.isNormalNode(node)) {
      ...
      dispatchEvent(
          new NodeAddRemoveEvent(FXConnectorEvent.SVEventType.NODE_ADDED, getID(), svNode));
    }
  }
  ...
  private void removeNode(final Node node, final boolean removeVisibilityListener) {
    if (SCUtils.isNormalNode(node)) {
      ...
      dispatchEvent(
          new NodeAddRemoveEvent(
              FXConnectorEvent.SVEventType.NODE_REMOVED, getID(), createNode(node)));
    }
  }

  private void updateListeners(
      final Node node, final boolean add, final boolean removeVisibilityListener) {
    if (add) {
      if (SCUtils.isNormalNode(node)) {
        ...
        if (configuration.isEventLogEnabled()) node.addEventFilter(Event.ANY, traceEventHandler);
  ...
  private Consumer<CSSFXEvent<?>> eventTranslator =
      new Consumer<CSSFXEvent<?>>() {
        @Override
        public void accept(CSSFXEvent<?> event) {
          ...
          CSSFXEvent.EventType et = event.getEventType();
          ...
          switch (et) {
            ...
            case STYLESHEET_REPLACED:
              ...
              FXConnectorEvent.SVEventType type = fromCSSEvent(et);
              ...
              EvCSSFXEvent e =
                  new EvCSSFXEvent(
                      ...
                      node,
                      ms.getOriginalURI(),
                      (ms.getSource() == null) ? null : ms.getSource().toString());
        ...
        private FXConnectorEvent.SVEventType fromCSSEvent(CSSFXEvent.EventType et) {
          return switch (et) {
            case STYLESHEET_MONITORED, STYLESHEET_REMOVED, STYLESHEET_REPLACED ->
                FXConnectorEvent.SVEventType.CSS_ADDED;
            default -> null;
          };
        }
      };
