...
class SCUtils {
    ...
    static void removeScenicViewComponents(final Node target) {
        ...
        if (target instanceof Group g) {
            final List<Node> nodes = g.getChildren();
            nodes.removeIf(node -> !isNormalNode(node));
        }
        if (target instanceof Pane t) {
            val nodes = t.getChildren();
            nodes.removeIf(node -> !isNormalNode(node));
        }
    }
    ...
    static Node findNode(final Node target, final int nodeUniqueID) {
        ...
        final List<Node> children = NodeUtil.getChildren(target);
    ...
    static Node getHoveredNode(final Configuration configuration, final Node target, final double x, final double y) {
        ...
        final List<Node> childrens = NodeUtil.getChildren(target);
