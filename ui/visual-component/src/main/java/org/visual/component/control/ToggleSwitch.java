...
public class ToggleSwitch {
  ...
  public ToggleSwitch(double radius, double length) {
    ...
    unselectedColorNode =
        new AnimationNode<>(
            "unselected", new ColorData(Theme.current().toggleSwitchUnselectedButtonColor()));
    unselectedHoverColorNode =
        new AnimationNode<>(
            "hover", new ColorData(Theme.current().toggleSwitchUnselectedButtonHoverColor()));
    selectedColorNode =
        new AnimationNode<>(
            "selected", new ColorData(Theme.current().toggleSwitchSelectedButtonColor()));
    colorAnimation =
    ...
    posAnimation =
        AnimationGraphBuilder.simpleTwoNodeGraph(unselectedPosNode, selectedPosNode, 150)
            .setApply((from, to, d) -> button.setLayoutX(d.value))
            .build(unselectedPosNode);

    trayUnselectedNode =
        new AnimationNode<>(
            "unselected", new ColorData(Theme.current().toggleSwitchUnselectedTrayColor()));
    traySelectedNode =
        new AnimationNode<>(
            "selected", new ColorData(Theme.current().toggleSwitchSelectedTrayColor()));
    trayAnimation =
        AnimationGraphBuilder.simpleTwoNodeGraph(trayUnselectedNode, traySelectedNode, 150)
