package org.visual.component.control.globalscreen;
...
public class GlobalScreenUtils {
  ...
  public static void releaseJNativeHookNativeToLibraryPath(
      InputStream inputStream, Function<Stream<File>, Optional<File>> selectFunc)
      throws IOException {
    String suffix;
    if (OS.OS == OS.WINDOWS) {
      suffix = ".dll";
    ...
    var stream =
        Arrays.stream(javaLibraryPath.split(File.pathSeparator))
            .filter(s -> !s.isBlank())
            .map(File::new)
  ...
  public static synchronized void disable(Object key) {
    ...
    if (n == null) {
      log.info(
          "GlobalScreenUtils.disable is called with "
              + key
              + ", but it's not enabled with this key before");
      return;
    }
    n -= 1;
    if (n == 0) {
