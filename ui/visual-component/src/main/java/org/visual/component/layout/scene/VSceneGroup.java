...
public class VSceneGroup {
  ...
  public VSceneGroup(@NotNull VScene initialMainScene, VSceneGroupInitParams initParams) {
    ...
    initialMainScene.changeListeners.add(
        ObserveUtil.observeWidth(root, initialMainScene.getNode()));
    initialMainScene.changeListeners.add(
        ObserveUtil.observeHeight(root, initialMainScene.getNode()));
  ...
  public void addScene(
    ...
    if (scene.role.manageWidth) {
      scene.changeListeners.add(ObserveUtil.observeWidth(root, scene.getNode()));
    }
    if (scene.role.manageHeight) {
      scene.changeListeners.add(ObserveUtil.observeHeight(root, scene.getNode()));
    }
    if (scene.role.centralWidth) {
      scene.changeListeners.add(ObserveUtil.observeWidthCenter(root, scene.getNode()));
    }
    if (scene.role.centralHeight) {
      scene.changeListeners.add(ObserveUtil.observeHeightCenter(root, scene.getNode()));
    }
