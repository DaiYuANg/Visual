...
public class VScene implements NodeWithVScrollPane {
  ...
  final AnimationNode<XYZTData> stateBottom =
      new AnimationNode<>("bottom", new XYZTData(0, 1, 1, 0));
  final AnimationNode<XYZTData> stateLeft = new AnimationNode<>("left", new XYZTData(-1, 0, 1, 0));
  final AnimationNode<XYZTData> stateCenterShown =
      new AnimationNode<>("center", new XYZTData(0, 0, 1, 1));
  final AnimationNode<XYZTData> stateFaded =
      new AnimationNode<>("faded", new XYZTData(0, 0, Float.MIN_VALUE * 2, 0));
  final AnimationNode<XYZTData> stateRemoved =
      new AnimationNode<>("removed", new XYZTData(0, 0, 1, 0), this::transferredToRemovedState);

  final AnimationGraph<XYZTData> animationGraph =
      new AnimationGraphBuilder<XYZTData>()
          .addNode(stateTop)
          .addNode(stateRight)
          .addNode(stateBottom)
          .addNode(stateLeft)
  ...
  public VScene(VSceneRole role) {
    ...
    ObserveUtil.observeWidthHeight(root, scrollPane.getNode());
  ...
  public void enableAutoContentWidth() {
    ObserveUtil.observeWidth(root, content, -1);
  }

  public void enableAutoContentHeight() {
    ObserveUtil.observeHeight(root, content, -1);
  }
  ...
  private void applyAnimation(
      AnimationNode<XYZTData> from, AnimationNode<XYZTData> to, XYZTData data) {
  ...
  private void animationStateTransferBegin(
      AnimationNode<XYZTData> from, AnimationNode<XYZTData> to) {
  ...
  private void updateBackgroundImagePos() {
    ...
    if (backgroundImage.getImage() == null) return;
