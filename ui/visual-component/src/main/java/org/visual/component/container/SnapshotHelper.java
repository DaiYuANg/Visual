...
public class SnapshotHelper {
  ...
  /**
   * Snapshots the blurred content below a {@link FrostyBox} and crops the result so that it fits
   * the size of the box
   *
  ...
  private int properValue(double value, double max) {
    if (value < 0) return 0;
    if (max < 0) return 0;
    // This method is called multiple times, therefore avoiding Math.min calls improves the
    // general
    // performance.
