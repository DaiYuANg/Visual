...
public class VScrollPane implements NodeWithVScrollPane {
  ...
  private final VerticalScrollBarImpl scrollBarV =
      new VerticalScrollBarImpl() {
        {
          setMouseTransparent(true);
        }
      };
  private final HorizontalScrollBarImpl scrollBarH =
      new HorizontalScrollBarImpl() {
        {
          setMouseTransparent(true);
        }
      };
  ...
  private final AnimationNode<DoubleData> animationHide =
      new AnimationNode<>("hide", new DoubleData(0));
  private final AnimationNode<DoubleData> animationShow =
      new AnimationNode<>("show", new DoubleData(1));
  private final AnimationGraph<DoubleData> animationHideShow =
      AnimationGraphBuilder.simpleTwoNodeGraph(animationHide, animationShow, 300)
          .setApply(
              (from, to, d) -> {
          ...
          .build(animationHide);
  @Getter @Setter private ScrollDirection scrollDirection;
            ...
            e -> {
              ...
              var ll =
                  scrollDirection == ScrollDirection.HORIZONTAL
                      ? viewport.getContentWidth()
                      : viewport.getContentHeight();
              if (ll == 0) return;
  ...
  public static VScrollPane makeHorizontalScrollPaneToManage(NodeWithVScrollPane node) {
    ...
    Runnable update =
        () ->
            node.getScrollPane()
                .setVerticalScrollBarLayoutX(
                    -pane.getHpos()
                        + pane.getNode().getWidth()
                        - VScrollPane.SCROLL_WIDTH
