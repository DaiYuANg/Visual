...
public class VSceneGroup {
  ...
  @Getter private VScene currentMainScene;
  @Getter private VScene nextMainScene = null;
  ...
  public VSceneGroup(@NotNull VScene initialMainScene, VSceneGroupInitParams initParams) {
    ...
    try {
      ...
      if (!res)
        throw new IllegalArgumentException(
            "unable to show because beforeShowing() method returned false");
    ...
    initialMainScene.changeListeners.add(FXUtils.observeWidth(root, initialMainScene.getNode()));
    initialMainScene.changeListeners.add(FXUtils.observeHeight(root, initialMainScene.getNode()));
  ...
  public void addScene(
      VScene scene, VSceneHideMethod defaultHideMethod, VSceneAddParams addParams) {
    ...
    if (defaultHideMethod == null) {
      if (scene.role != VSceneRole.MAIN) {
        throw new IllegalArgumentException(
            "non-main scene must use addScene(VScene, VSceneHideMethod) to add the scene to scene group");
      }
    }

    if (scene.role.manageWidth) {
      scene.changeListeners.add(FXUtils.observeWidth(root, scene.getNode()));
    }
    if (scene.role.manageHeight) {
      scene.changeListeners.add(FXUtils.observeHeight(root, scene.getNode()));
    }
    if (scene.role.centralWidth) {
      scene.changeListeners.add(FXUtils.observeWidthCenter(root, scene.getNode()));
    }
    if (scene.role.centralHeight) {
      scene.changeListeners.add(FXUtils.observeHeightCenter(root, scene.getNode()));
    }
  ...
  public void show(VScene scene, VSceneShowMethod method) {
    ...
    if (animatingHide.contains(scene)) {
      ...
      if (precheckShow(scene)) {
        scene.animationGraph.play(
            scene.stateCenterShown, Callback.ofIgnoreExceptionFunction(v -> postShowing(scene)));
      }
      return;
    }
    ...
    Callback<Void, Exception> cb =
        Callback.ofIgnoreExceptionFunction(v -> showStep2(scene, method));
    switch (method) {
      case FROM_TOP:
  ...
  private void showStep2(VScene scene, @NotNull VSceneShowMethod method) {
    switch (method) {
      case FROM_TOP:
        scene.x0 = 0;
        scene.y0 = 0;
  ...
  private void hideSkipCheck(VScene scene, VSceneHideMethod method) {
    if (animatingShow.contains(scene)) {
      ...
      scene.animationGraph.play(
          scene.stateRemoved, Callback.ofIgnoreExceptionFunction(v -> postHiding(scene)));
      return;
    }
    if (animatingHide.contains(scene)) {
      return;
  ...
  private void animateCover(@NotNull VScene scene, double p) {
    if (scene.cover != null) {
  ...
  private void doAnimate(VScene scene, AnimationNode<XYZTData> keyNode, boolean isShowing) {
    ...
    Callback<Void, Exception> cb =
        Callback.ofIgnoreExceptionFunction(
            v -> {
  ...
  public boolean isShowing(VScene scene) {
    return scene == currentMainScene
        || showingScenes.contains(scene)
        || animatingShow.contains(scene);
  }
