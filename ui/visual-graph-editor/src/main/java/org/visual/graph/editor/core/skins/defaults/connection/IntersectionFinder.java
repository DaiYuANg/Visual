...
public class IntersectionFinder {
  ...
  /**
   * Finds the intersection points of the connection with all other connections that are in front of
   * / behind it.
   *
  ...
  public static double[][] find(
      final GConnectionSkin pSkin,
      final Map<GConnectionSkin, Point2D[]> allPoints,
    ...
    for (int i = 0; i < points.length - 1; i++) {
      ...
      final double[] segmentIntersections =
          findSegmentIntersections(pSkin, allPoints, behind, i, isHorizontal);
   ...
   */
  private static double[] findSegmentIntersections(
      final GConnectionSkin connection,
      final Map<GConnectionSkin, Point2D[]> allPoints,
      final boolean behind,
      final int index,
          ...
          if (GeometryUtils.checkIntersection(a, b, c, d)) {
            ...
            if (resultLen >= segmentIntersections.length) {
              segmentIntersections =
                  Arrays.copyOf(segmentIntersections, segmentIntersections.length + 5);
            }
  ...
  /**
   * Filters out some connections, because we want to either ignore connections in front or behind.
   * We only want one of the connections at an intersection to draw a detour graphic.
   *
   * @param behind {@code true} to leave in connections behind this one and filter out those in
   *     front
   * @return a stream of connections with some filtered out
   */
  private static boolean filterConnection(
      final @NotNull GConnectionSkin connection,
      final boolean behind,
   ...
   */
  private static boolean checkIfBehind(
      final GConnectionSkin skin, final GConnectionSkin otherSkin) {
