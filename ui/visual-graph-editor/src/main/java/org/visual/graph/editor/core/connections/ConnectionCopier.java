...
public final class ConnectionCopier {
   ...
   *
   * <p>Connections between nodes in the <b>keys</b> of the input map are copied (including their
   * joint information) and the new connections are set inside the corresponding nodes in the
   * <b>values</b> of the input map.
   *
   * <p>The new connection information is set <em>directly</em>. EMF commands are not used
   *
   * @param copies a map that links source nodes to their copies in its key-value pairs
   * @return the list of created connections
   */
  @Contract("_ -> new")
  public static @NotNull List<GConnection> copyConnections(
      final @NotNull Map<GNode, GNode> copies) {
   ...
   */
  private static @Nullable GNode getOpposingNode(
      final GConnector connector, final @NotNull GConnection connection) {
