...
import org.visual.graph.editor.core.skins.defaults.utils.ConnectionCommands;
 ...
 *
 * <p>Namely, the creation, removal, and repositioning of connections.
 */
public class ConnectorDragManager {
  ...
  /**
   * Consume the Event so the parent container (ResizableBox/DraggableBox) does not move on
   * connection detach
   */
  ...
  /**
   * Creates a new {@link ConnectorDragManager}. Only one instance should exist per {@link
   * DefaultGraphEditor} instance.
   *
   * @param skinLookup the {@link SkinLookup} used to look up connector and tail skins
   * @param connectionEventManager the {@link ConnectionEventManager} used to notify users of
   *     connection events
   * @param view the {@link GraphEditorView} to which tail skins will be added and removed during
   *     drag events
   */
  public ConnectorDragManager(
  ...
  /**
   * Clears all parameters that track things like what connector is currently hovered over, and so
   * on.
   */
  ...
  public void removeConnector(final GConnector pConnectorToRemove) {
    ...
    if (connectorSkin != null) {
      ...
      if (root != null) {
        ...
        removeSingleEventHandler(
            root, mouseDragReleasedHandlers, MouseDragEvent.MOUSE_DRAG_RELEASED);
    ...
    if (sourceConnector == pConnectorToRemove
        || targetConnector == pConnectorToRemove
        || removalConnector == pConnectorToRemove) {
      clearTrackingParameters();
    }
  }
  ...
  private static <T extends Event> void removeSingleEventHandler(
      final Node node,
      final @NotNull Map<Node, EventHandler<T>> eventHandlerMap,
  ...
  private void addMouseHandlers(final GConnector connector) {
    ...
    if (connectorSkin != null) {
      ...
      final EventHandler<MouseEvent> newMouseEnteredHandler =
          event -> handleMouseEntered(event, connector);
      final EventHandler<MouseEvent> newMouseReleasedHandler = this::handleMouseReleased;

      final EventHandler<MouseEvent> newDragDetectedHandler =
          event -> handleDragDetected(event, connectorSkin);
      final EventHandler<MouseEvent> newMouseDraggedHandler =
          event -> handleMouseDragged(event, connector);
  ...
  private void handleMouseReleased(final MouseEvent event) {
    ...
    if (targetConnector != null
        && (targetConnectorSkin = skinLookup.lookupConnector(targetConnector)) != null) {
   ...
   *
   * @param pEvent a drag-detected event
   * @param connectorSkin the {@link GConnectorSkin} on which this event occurred
   */
  private void handleDragDetected(
      final @NotNull MouseEvent pEvent, final GConnectorSkin connectorSkin) {
  ...
  private void handleDragExited(
      final MouseEvent event, final @NotNull GConnectorSkin connectorSkin) {
   ...
   * @param connectorSkin the {@link GConnectorSkin} on which this event occurred
   */
  private void handleDragReleased(
      final @NotNull MouseEvent event, final GConnectorSkin connectorSkin) {
    if (event.isConsumed()) {
      return;
    }
    ...
    final GConnector connector = connectorSkin.getItem();
    if (validator.prevalidate(sourceConnector, connector)
        && validator.validate(sourceConnector, connector)) {
   ...
   * @return {@code true} if a connection can be created from the given {@link GConnector}, {@code
   *     false} if not
   */
  private boolean checkCreatable(final GConnector connector) {
    return connector != null
        && connector.eContainer() instanceof GNode
        && checkEditable()
   ...
   *
   * @param connector a {@link GConnector} instance
   * @return {@code true} if a connection can be removed from the given {@link GConnector}, {@code
   *     false} if not
   */
  private boolean checkRemovable(final GConnector connector) {
    return checkEditable()
        && !connector.getConnections().isEmpty()
        && connector.isConnectionDetachedOnDrag();
  }

  private boolean checkEditable() {
    return getEditorProperties() != null
        && !getEditorProperties().isReadOnly(EditorElement.CONNECTOR);
  }
   ...
   *
   * <p>This will trigger the model listener and cause everything to be reinitialized.
   *
  ...
  private void addConnection(final GConnector source, final GConnector target) {
    ...
    ConnectionCommands.addConnection(
        model, source, target, connectionType, joints, connectionEventManager);
  }

  /**
   * Detaches the first connection from the given connector - i.e. removes the connection and
   * replaces it with a tail.
   *
  ...
  private void detachConnection(final MouseEvent event, final GConnector connector) {
    ...
    for (final GConnection connection : connections) {
      ...
      if ((updateConnectorSkin = skinLookup.lookupConnector(connector)) == null
          || updateConnectorSkin != connectorSkin
          || connectorCount != getConnectorCount(connector)) {
        // business logic decided to remove this connection or structurally change the
        // parent node
        continue;
      }
  ...
  private static int getConnectorCount(final GConnector pConnector) {
    return pConnector == null || pConnector.getParent() == null
        ? 1
        : pConnector.getParent().getConnectors().size();
  }

  private GConnector getOpposingConnector(
      final @NotNull GConnection pConnection, final GConnector pConnector) {
