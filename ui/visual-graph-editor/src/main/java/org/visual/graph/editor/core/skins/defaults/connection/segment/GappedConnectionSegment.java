...
public class GappedConnectionSegment extends ConnectionSegment {
   ...
   */
  public GappedConnectionSegment(
  ...
  private void addHGapTo(final double x) {
    getPathElements()
        .add(
            new MoveTo(
                GeometryUtils.moveOffPixel(x), GeometryUtils.moveOffPixel(getStart().getY())));
  }
  ...
  private void addVGapTo(final double y) {
    getPathElements()
        .add(
            new MoveTo(
                GeometryUtils.moveOffPixel(getStart().getX()), GeometryUtils.moveOffPixel(y)));
  }
}
