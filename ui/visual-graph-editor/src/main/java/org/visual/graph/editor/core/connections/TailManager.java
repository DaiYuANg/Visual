...
public class TailManager {
   ...
   * @param pNewSource
   * @param pEvent the mouse event responsible for creating the tail
   */
  public void updateToNewSource(
      final List<Point2D> pJointPositions, final GConnector pNewSource, final MouseEvent pEvent) {
    cleanUp();
  ...
  /**
   * Snaps the position of the tail to show the position the connection itself would take if it
   * would be created.
   *
   ...
   * Cleans up.
   *
   * <p>Called at the end of a drag gesture or during initialization. Removes any tail from the view
   * and resets tracking parameters.
   */
  public void cleanUp() {
