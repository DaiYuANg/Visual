 ...
 */
package org.visual.graph.editor.core.model;
...
public class ModelLayoutUpdater {
  ...
  private final EventHandler<MouseEvent> mouseReleasedHandlerNode =
      event -> elementMouseReleased(EditorElement.NODE);
  private final EventHandler<MouseEvent> mouseReleasedHandlerJoint =
      event -> elementMouseReleased(EditorElement.JOINT);

  /**
   * Creates a new model layout updater. Only one instance should exist per graph editor instance.
   *
   * @param pSkinLookup the {@link SkinLookup} used to lookup skins
   * @param pModelEditingManager the {@link ModelEditingManager} used to update the model values
   */
  public ModelLayoutUpdater(
      final SkinLookup pSkinLookup,
      final ModelEditingManager pModelEditingManager,
