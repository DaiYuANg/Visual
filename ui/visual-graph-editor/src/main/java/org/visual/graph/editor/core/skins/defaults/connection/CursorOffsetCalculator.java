...
public class CursorOffsetCalculator {
  ...
  public Point2D getOffset(final double cursorSceneX, final double cursorSceneY) {
    ...
    IntStream.range(1, path.getElements().size())
        ...
        .forEach(
            pathElement -> calculateOffset(pathElement, cursorSceneX, cursorSceneY, offsetBound));
   ...
   *
   * <p>If the offset is smaller than the current minimum variable, its value will be updated.
   *
  ...
  private void calculateOffset(
      ...
      final double cursorSceneX,
      final double cursorSceneY,
    ...
    if (pathElement instanceof HLineTo hLineTo) {
      ...
      final boolean cursorInRangeY =
          GeometryUtils.checkInRange(currentSceneY, nextSceneY, cursorSceneY);
