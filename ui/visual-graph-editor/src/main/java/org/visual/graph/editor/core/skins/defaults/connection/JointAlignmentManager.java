...
import org.visual.graph.editor.api.GNodeSkin;
...
/**
 * Responsible for how joints align to adjacent joints when dragged close enough to them in the
 * default connection skin.
 */
public class JointAlignmentManager {
  ...
  /**
   * -- SETTER -- Sets the skin directory instance for this graph editor.
   *
   * @param skinLookup the {@link SkinLookup} instance for this graph editor
   */
  @Setter private SkinLookup skinLookup;
   ...
   *
   * <p>This is a mouse-pressed handler which checks for appropriate adjacent joints and sets
   * alignment targets in the joint's {@link DraggableBox} root node, so that the joint will align
   * to adjacent joints when dragged near them.
   *
  ...
  public void addAlignmentHandlers(final @NotNull List<GJointSkin> jointSkins) {
    ...
    final Map<GJointSkin, EventHandler<MouseEvent>> oldAlignmentHandlers =
        new HashMap<>(alignmentHandlers);
    ...
    jointSkins.forEach(
        jointSkin -> {
          ...
          final EventHandler<MouseEvent> newHandler =
              event -> {
  ...
  /**
   * Adds horizontal alignment targets to the given joint skin based on the positions of
   * neighbouring joints.
   *
   * @param jointSkin the current joint skin
   * @param jointSkins all joint skin instances belonging to a connection
   */
  private void addHorizontalAlignmentTargets(
      final GJointSkin jointSkin, final @NotNull List<GJointSkin> jointSkins) {
    ...
    // First check for existence of previous vertical segment that will not move when this joint
    // is
    // dragged.
    if (isPreviousVerticalSegmentStationary(index, jointSkins)) {
      if (index == 1) {
        alignmentValuesX.add(jointSkins.getFirst());
      } else if (index > 1) {
        alignmentValuesX.add(jointSkins.get(index - 2));
      }
    }

    // Then check for existence of next vertical segment that will not move when this joint is
    // dragged.
  ...
  /**
   * Adds vertical alignment targets to the given joint skin based on the positions of neighbouring
   * joints.
   *
   * @param jointSkin the current joint skin
   * @param jointSkins all joint skin instances belonging to a connection
   */
  private void addVerticalAlignmentTargets(
      final GJointSkin jointSkin, final @NotNull List<GJointSkin> jointSkins) {
    final int index = jointSkins.indexOf(jointSkin);
    final int count = jointSkins.size();
  ...
  /**
   * Checks whether the previous vertical horizontal segment will remain stationary when the current
   * joint is dragged.
   *
   ...
   */
  private boolean isPreviousVerticalSegmentStationary(
      final int index, final List<GJointSkin> jointSkins) {

    final boolean firstSegmentHorizontal =
        RectangularConnections.isSegmentHorizontal(connection, 0);
  ...
  /**
   * Checks whether the next vertical segment will remain stationary when the current joint is
   * dragged.
   *
   ...
   */
  private boolean isNextVerticalSegmentStationary(
      final int index, final @NotNull List<GJointSkin> jointSkins) {

    final int count = jointSkins.size();
    final boolean lastSegmentHorizontal =
        RectangularConnections.isSegmentHorizontal(connection, count);
  ...
  /**
   * Checks whether the previous horizontal segment will remain stationary when the current joint is
   * dragged.
   *
   * @param index the index of the current joint
   * @param jointSkins the list of joint skins for the joint's connection
   * @return {@code true} if the previous horizontal segment of the connection will remain
   *     stationary
   */
  private boolean isPreviousHorizontalSegmentStationary(
      final int index, final List<GJointSkin> jointSkins) {
  ...
  /**
   * Checks whether the next horizontal segment will remain stationary when the current joint is
   * dragged.
   *
   * @param index the index of the current joint
   * @param jointSkins the list of joint skins for the joint's connection
   ...
   */
  private boolean isNextHorizontalSegmentStationary(
      final int index, final @NotNull List<GJointSkin> jointSkins) {
   ...
   */
  private boolean isJointPairStationary(
      final int index,
