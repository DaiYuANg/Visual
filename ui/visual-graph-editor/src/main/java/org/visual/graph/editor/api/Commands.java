...
public class Commands {
  ...
  private static final EReference CONNECTOR_CONNECTIONS =
      GraphPackage.Literals.GCONNECTOR__CONNECTIONS;
   ...
   *
   * <p>The node's x, y, width, and height values should be set before calling this method.
   *
   ...
   *
   * <p>Also removes any connections that were attached to the node.
   *
    ...
    if (editingDomain != null) {
      ...
      for (final GConnection connection : connectionsToDelete) {
        ...
        if (!node.equals(source.getParent())) {
          command.append(
              RemoveCommand.create(editingDomain, source, CONNECTOR_CONNECTIONS, connection));
        }

        if (!node.equals(target.getParent())) {
          command.append(
              RemoveCommand.create(editingDomain, target, CONNECTOR_CONNECTIONS, connection));
        }
      }
  ...
  public static void clear(final GModel model) {
    ...
    if (editingDomain != null) {
      ...
      command.append(
          RemoveCommand.create(editingDomain, model, CONNECTIONS, model.getConnections()));
  ...
  public static void clearConnectors(final GModel model, final List<GNode> nodes) {
    ...
    if (editingDomain != null) {
      ...
      for (final GNode node : nodes) {
        command.append(
            RemoveCommand.create(editingDomain, node, NODE_CONNECTORS, node.getConnectors()));
   ...
   *
   * <p>This method adds set operations to the given compound command but does <b>not</b> execute
   * it.
   *
   ...
   */
  public static void updateLayoutValues(
      final CompoundCommand command, final GModel model, final SkinLookup skinLookup) {
    final EditingDomain editingDomain = getEditingDomain(model);
    ...
    if (editingDomain != null) {
      for (final GNode node : model.getNodes()) {
        ...
        if (nodeSkin != null && checkNodeChanged(node, nodeSkin)) {
          ...
          command.append(
              SetCommand.create(editingDomain, node, NODE_HEIGHT, nodeRegion.getHeight()));
        }
      }
      ...
      if (checkConnectorChanged(connector, connectorPosition)) {
        command.append(
            SetCommand.create(
                editingDomain,
                connector,
                GraphPackage.Literals.GCONNECTOR__X,
                connectorPosition.getX()));
        command.append(
            SetCommand.create(
                editingDomain,
                connector,
                GraphPackage.Literals.GCONNECTOR__Y,
  ...
  /**
   * Checks if a connector's JavaFX region has different layout values than those currently stored
   * in the model.
   *
   * @param connector the model instance for the connector
   * @return {@code true} if any layout value has changed, {@code false if not}
   */
  private static boolean checkConnectorChanged(
      final GConnector connector, final Point2D connectorPosition) {
  ...
  /**
   * Checks if a node's JavaFX region has different layout values than those currently stored in the
   * model.
   *
  ...
  /**
   * Checks if a joint's JavaFX region has different layout values than those currently stored in
   * the model.
   *
   ...
   *
   * <p>Logs an error if none is found.
   *
  ...
  private static EditingDomain getEditingDomain(final GModel model) {
    ...
    if (editingDomain == null) {
      log.error(
          "No editing domain found for this model. Maybe it hasn't been set inside a graph editor?");
    }
