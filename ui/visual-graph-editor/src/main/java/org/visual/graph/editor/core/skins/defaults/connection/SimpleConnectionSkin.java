 ...
 *
 * <p>Shows a rectangular connection shape based on the positions of its joints. Shows a graphical
 * effect at points where the connection intersects other connections.
 */
public class SimpleConnectionSkin extends GConnectionSkin {
   ...
   *
   * <p>By default small gaps are drawn points where the connection passes <b>under</b> other
   * connections. However it is also possible to draw detours (small semicircles) at points where
   * the connection passes <b>over</b> others.
   *
   * <p>To activate this functionality, add this key to the graph editor's custom properties with
   * the value "true". Do <b>NOT</b> mix 'detoured' and 'gapped' connection skins in the same graph,
   * it will look bad.
   */
  ...
  public void draw(final Map<GConnectionSkin, Point2D[]> allPoints) {
    ...
    // If we are showing detours, get all intersections with connections *behind* this one.
    // Otherwise in front.
  ...
  private void removeOldRectangularConstraints() {
    IntStream.range(0, jointSkins.size() - 1)
        .forEach(
            i -> {
  ...
  /**
   * Adds constraints to the connection's joints in order to keep the connection rectangular in
   * shape.
   */
  ...
  /**
   * Checks the position of the first and last joints and makes sure they are aligned with their
   * adjacent connectors.
   *
   * @param points all points that the connection should pass through (both connector and joint
   *     positions)
   */
  private void checkFirstAndLastJoints(final Point2D[] points) {
    ...
    alignJoint(
        points, RectangularConnections.isSegmentHorizontal(getItem(), points.length - 2), false);
  }

  /**
   * Aligns the first or last joint to have the same vertical or horizontal position as the start or
   * end point.
   *
   * @param points the list of points in this connection
   * @param vertical {@code true} to align in the vertical (y) direction, {@code false} for
   *     horizontal (x)
   * @param start {@code true} to align the first joint to the start, {@code false} for the last
   *     joint to the end
   */
  private void alignJoint(final Point2D[] points, final boolean vertical, final boolean start) {
    ...
    if (vertical) {
      ...
      final double newJointLayoutY =
          GeometryUtils.moveOnPixel(newJointY - jointSkin.getHeight() / 2);
      ...
      final double newJointLayoutX =
          GeometryUtils.moveOnPixel(newJointX - jointSkin.getWidth() / 2);
  ...
  /**
   * Checks whether the custom property has been set to show detours instead of gaps when
  ...
  private boolean checkShowDetours() {
    ...
    final String value =
        getGraphEditor().getProperties().getCustomProperties().get(SHOW_DETOURS_KEY);
