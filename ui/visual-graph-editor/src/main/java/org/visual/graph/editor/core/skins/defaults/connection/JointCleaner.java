...
public class JointCleaner {
  ...
  /**
   * Adds handlers to remove all joints that are on top of each other after a mouse-released
   * gesture.
   *
          ...
          event -> {
            ...
            if (Boolean.FALSE.equals(jointsToCleanUp.isEmpty())) {
              final EditingDomain editingDomain =
                  AdapterFactoryEditingDomain.getEditingDomainFor(mConnection);
   ...
   *
   * @param jointPositions a list of {@link Point2D} instances containing the x and y values of the
   *     joints
