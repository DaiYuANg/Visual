...
import org.visual.graph.editor.core.connections.ConnectorDragManager;
import org.visual.graph.editor.core.model.DefaultModelEditingManager;
import org.visual.graph.editor.core.model.ModelLayoutUpdater;
import org.visual.graph.editor.core.model.ModelSanityChecker;
import org.visual.graph.editor.core.selections.DefaultSelectionManager;
import org.visual.graph.editor.core.skins.SkinManager;
import org.visual.graph.editor.core.view.ConnectionLayouter;
import org.visual.graph.editor.core.view.GraphEditorView;
import org.visual.graph.editor.core.view.impl.DefaultConnectionLayouter;
import org.visual.graph.editor.model.*;

/**
 * The central controller class for the default graph editor implementation.
 *
 * <p>Responsible for using the {@link SkinManager} to create all skin instances for the current
 * {@link GModel}, and adding them to the {@link GraphEditorView view}.
 *
 * <p>Also responsible for creating all secondary managers like the {@link ConnectorDragManager} and
 * reinitializing them when the model changes.
 *
 * <p>The process of synchronizing is rather complicated in case more than one model is part of the
 * resource set:
 *
 * <ol>
 *   <li>register listener on every model in the resource set (with an {@link EContentAdapter}
 *   <li>receive notifications
 *   <li>put notification into queue
 *   <li>{@link #process() process queue} on every reload and/or command stack change
 * </ol>
 *
 * This procedure (processing a chunk of notifications on command stack change or {@link
 * GraphEditor#reload()} is a very safe way to determine a valid package of changes.
 *
 * <p>This implementation is thread safe: It is able to process notifications in parallel and
 * processes them in chunks on the FX Thread.
 */
@Slf4j
public class GraphEditorController<E extends GraphEditor> {
  ...
  private final Map<EStructuralFeature, Consumer<Notification>> mHandlersByFeature =
      new HashMap<>();
  ...
  private final ModelEditingManager mModelEditingManager =
      new DefaultModelEditingManager(mCommandStackListener);
  ...
  /**
   * While processing a {@link Notification} from EMF it might trigger other changes which will
   * eventually lead to an infinite update cycle.<br>
   * When this flag is {@code true} we are currently processing a batch of updates. All new
   * notifications will be put in the queue but are processed later on.
   */
  private boolean mProcessing = false;

  /**
   * Creates a new controller instance. Only one instance should exist per {@link GraphEditor}
   * instance.
   *
  ...
  public GraphEditorController(
      final E pEditor,
      final SkinManager pSkinManager,
      final GraphEditorView pView,
      final ConnectionEventManager pConnectionEventManager,
  ...
  private void initDefaultListeners() {
    registerChangeListener(
        GraphPackage.Literals.GMODEL__NODES,
        e -> processNotification(e, this::addNode, this::removeNode));

    registerChangeListener(
        GraphPackage.Literals.GNODE__CONNECTORS,
    ...
    registerChangeListener(
        ...
        e -> {
          if (e.getNotifier() instanceof GNode n) {
            ...
            // luckily getNotifier() still returns the GNode where the connector was
            // removed from:
            markConnectorsDirty(n);
          }
        });
    registerChangeListener(
        GraphPackage.Literals.GMODEL__CONNECTIONS,
        e -> processNotification(e, this::addConnection, this::removeConnection));

    registerChangeListener(
        GraphPackage.Literals.GCONNECTION__JOINTS,
        e ->
            processNotification(
                e,
                (GJoint j) -> addJoint(j, e.getNotifier()),
    ...
    registerChangeListener(
        GraphPackage.Literals.GNODE__TYPE,
        e -> {
  ...
  public final void registerChangeListener(
      final EStructuralFeature pFeature, final Consumer<Notification> pHandler) {
   ...
   */
  public final void registerChangeListener(
      final int pNotificationType, final Consumer<Notification> pHandler) {
    Objects.requireNonNull(pHandler, "Notification Consumer may not be null!");
  ...
  private void modelChanged(final GModel pOldModel, final GModel pNewModel) {
    if (pOldModel != null) {
      final EditingDomain editingDomain =
          AdapterFactoryEditingDomain.getEditingDomainFor(pOldModel);
    ...
    if (pNewModel != null) {
      ...
      final EditingDomain editingDomain =
          AdapterFactoryEditingDomain.getEditingDomainFor(pNewModel);
      ...
      if (pNewModel instanceof InternalEObject ieo) {
        ...
        processFeatureChanged(
            new ENotificationImpl(
                ieo,
                Notification.ADD_MANY,
                GraphPackage.Literals.GMODEL__NODES,
                List.of(),
                List.copyOf(pNewModel.getNodes())));
        ...
        processFeatureChanged(
            new ENotificationImpl(
                ...
                GraphPackage.Literals.GMODEL__CONNECTIONS,
                List.of(),
                List.copyOf(pNewModel.getConnections())));
      } else {
      ...
      executeOnceWhenPropertyIsNonNull(
          mEditor.getView().sceneProperty(),
   ...
   * Only has an effect if the current Thread is the {@link Platform#isFxApplicationThread() FX
   * Application Thread}
   *
   * @since 09.02.2016
   */
  public final void process() {
    if (mProcessing
        || !Platform.isFxApplicationThread()) // prevent GUI updates outside the FX Application
    // Thread
    {
      return;
    }
  ...
  /**
   * @param pNode {@link GNode} of which the {@link GConnector connectors} have been changed
   * @since 15.03.2019
   */
   ...
   *
   * @param validator a {@link GConnectorValidator} implementation, or null to use the default
   */
  ...
  /**
   * Delegates the contents of the given {@link Notification} to the given {@link Consumer
   * consumers}:
   *
   * <ul>
   *   <li>In case of {@link Notification#ADD} or {@link Notification#ADD_MANY}, the add consumer
   *       will be called with each added element
   *   <li>In case of {@link Notification#REMOVE} or {@link Notification#REMOVE_MANY}, the remove
   *       consumer will be called with each added element
   * </ul>
   *
   * IMPORTANT: The added/removed values are casted without any checks, any one calling this method
   * should take extra care to not mix wrong types!
   *
   ...
   * @since 15.03.2019
   */
  protected static <T> void processNotification(
      final Notification pNotification, final Consumer<T> pAdd, final Consumer<T> pRemove) {
  ...
  /**
   * Attaches a value listener to the given {@link ObservableValue} and when the value changes to a
   * non-{@code null} value the given {@link Consumer} will be invoked with the new value and the
   * listener will be removed.
   *
   * <p>NOTE: If the {@link ObservableValue} already has a non-{@code null} value the {@link
   * Consumer} will be invoked directly.
   *
   * <p>This proves useful for linking things together before a property is necessarily set.
   *
   * @param pProperty {@link ObservableValue} to observe
   * @param pConsumer {@link Consumer} to call with the first non-{@code null} value
   * @since 12.05.2017
   */
  private static <T> void executeOnceWhenPropertyIsNonNull(
      final ObservableValue<T> pProperty, final Consumer<T> pConsumer) {
    if (pProperty == null) {
      return;
    }
