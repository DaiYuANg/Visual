...
import org.visual.graph.editor.api.GSkin;
...
public class SelectionTracker {
   ...
   *
   * @param skinLookup the {@link SkinLookup}
   */
  ...
  private void selectedElementsChanged(
      final SetChangeListener.@NotNull Change<? extends EObject> change) {
  ...
  public List<GConnection> getSelectedConnections() {
    return selectedElements.stream()
        .filter(e -> e instanceof GConnection)
        .map(e -> (GConnection) e)
        .toList();
  }
