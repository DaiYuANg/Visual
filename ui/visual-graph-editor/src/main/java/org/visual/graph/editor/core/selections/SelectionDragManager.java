...
import org.visual.graph.editor.api.SelectionManager;
...
public class SelectionDragManager {
  ...
  /**
   * Creates a new selection drag manager. Only one instance should exist per {@link
  ...
  private void masterMovedX(final double x) {
    ...
    IntStream.range(0, currentSelectedElements.size())
        .forEach(
            i -> {
  ...
  /**
   * Binds the positions of all selected objects to have a fixed position relative to the given
   * draggable box.
   *
   * @param pMaster the master {@link DraggableBox} that all selected objects should keep a fixed
  ...
  /**
   * Stores the current offset position of all selected objects with respect to the given master
   * region.
   *
   * @param master the master {@link Region} that all selected objects should keep a fixed position
   *     relative to
   */
   ...
   *
   * <p>Essentially it 'shrinks the walls' of the editor view temporarily during the drag operation,
   * so that it is not possible to drag any object outside the view if multiple objects are
   * selected.
   *
   * @param master the master {@link DraggableBox} that all selected objects should keep a fixed
   *     position relative to
   */
  private void setEditorBoundsForDrag(final @NotNull DraggableBox master) {

    final GraphEditorProperties propertiesForDrag =
        new GraphEditorProperties(view.getEditorProperties());

    final BoundOffsets maxOffsets = new BoundOffsets();

    currentSelectedElements.forEach(node -> addOffsets(master, node, maxOffsets));

    propertiesForDrag.setNorthBoundValue(
        propertiesForDrag.getNorthBoundValue() + maxOffsets.northOffset);
    propertiesForDrag.setSouthBoundValue(
        propertiesForDrag.getSouthBoundValue() + maxOffsets.southOffset);
    propertiesForDrag.setEastBoundValue(
        propertiesForDrag.getEastBoundValue() + maxOffsets.eastOffset);
    propertiesForDrag.setWestBoundValue(
        propertiesForDrag.getWestBoundValue() + maxOffsets.westOffset);

    master.setEditorProperties(propertiesForDrag);
  }

  /**
   * Calculates the offset between the given master and slave boxes and adds it to the maxOffsets
   * instance if it is larger than the current maximum value.
   *
   * @param master the master {@link DraggableBox} being dragged
   * @param slave the slave {@link DraggableBox} that is also selected and whose position is bound
   *     to the master
   * @param maxOffsets the {@link BoundOffsets} instance storing the current max offsets in all 4
   *     directions
   */
  private void addOffsets(
      final @NotNull DraggableBox master, final DraggableBox slave, final BoundOffsets maxOffsets) {
  ...
  /**
   * Adds listeners to the master region to update all slave regions accordingly when the master's
   * position changes.
   *
