...
import org.visual.graph.editor.api.window.AutoScrollingWindow;
import org.visual.graph.editor.api.window.GraphEditorMinimap;
import org.visual.graph.editor.model.GModel;

/**
 * A container for the graph editor.
 *
 * <p>This is intended for graphs that can be larger than the space available in the scene. The user
 * can pan around by right-clicking and dragging. A minimap can be shown to help with navigation.
 *
 * <p>Example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The graph editor container is a {@link Region} and can be added to the JavaFX scene graph in
 * the usual way.
 *
 * <p>When a {@link GraphEditor} is set inside this container, its view becomes <b>unmanaged</b> and
 * its width and height values are set to those in the {@link GModel} instance.
 */
public class GraphEditorContainer extends AutoScrollingWindow {

  /** default view stylesheet */
  private static final String STYLESHEET_VIEW =
      Objects.requireNonNull(GraphEditorContainer.class.getResource("/defaults.css"))
          .toExternalForm();

  private static final double MINIMAP_INDENT = 10;

  /** -- GETTER -- Returns the */
  @Getter private final GraphEditorMinimap minimap = new GraphEditorMinimap();

  private GraphEditor graphEditor;
  private final ChangeListener<GModel> modelChangeListener =
      (observable, oldValue, newValue) -> modelChanged(newValue);
