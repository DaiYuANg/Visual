 ...
 *
 * <p>A custom joint skin must extend this class. It <b>must</b> also provide a constructor taking
 * exactly one {@link GJoint} parameter.
 *
 * <p>The root JavaFX node of this skin is a {@link DraggableBox}.
 */
public abstract class GJointSkin extends GSkin<GJoint> {

  private final DraggableBox root =
      new DraggableBox(EditorElement.JOINT) {
   ...
   *
   * <p>The skin's layout values are loaded from the {@link GJoint} at this point.
   */
   ...
   *
   * <p>For robust behaviour this should return the correct width at all times. Note that
   * getRoot().getWidth() does not meet this condition as it will return 0 until the joint's node is
   * added to the scene graph and layed out.
   *
   ...
   *
   * <p>For robust behaviour this should return the correct height at all times. Note that
   * getRoot().getHeight() does not meet this condition as it will return 0 until the joint's node
   * is added to the scene graph and layed out.
   *
