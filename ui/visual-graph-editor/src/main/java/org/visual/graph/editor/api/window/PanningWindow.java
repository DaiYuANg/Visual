 ...
 *
 * <p>This window can be panned around relative to its content. Only the parts of the content that
 * are inside the window will be rendered. Everything outside it is clipped.
 */
public class PanningWindow extends Region {
  ...
  private final DoubleProperty contentX =
  ...
  private final DoubleProperty contentY =
      new SimpleDoubleProperty() {
   ...
   *
   * <p>The window cannot be panned outside the content. When the window 'hits the edge' of the
  ...
  /**
   * If there is no content at all or the content is smaller than the outer window we do not need to
   * pan at all
   *
   * @return {@code true} if the window should not be panned at all or {@code false} if the window
   *     can be panned
   */
   ...
   *
   * <p><b>Note: </b><br>
   * The current width and height values of the window and its content are used in this method. It
   * should therefore be called <em>after</em> the scene has been drawn.
   *
   ...
   *
   * @param pZoom new zoom factor
   * @param pPivotX the X coordinate about which point the scale occurs
   * @param pPivotY the Y coordinate about which point the scale occurs
   */
  ...
  protected void layoutChildren() {
    ...
    final double zoomFactor =
        theContent == null ? 1 : theContent.getLocalToSceneTransform().getMxx();
    scrollX.setMin(0);
  ...
  /**
   * Checks that the window bounds are completely inside the content bounds, and repositions if
   * necessary.
   *
   * <p>Also checks that the window position values are integers to ensure strokes are drawn
   * cleanly.
   */
   ...
   *
   * <p>Note that the content's {@code managed} attribute will be set to false. Its size must
   * therefore be set manually using the {@code resize()} method of the {@link Node} class.
   *
  ...
  private void handleScroll(final ScrollEvent pEvent) {
    ...
    // the event also gets synthesized from touch events, which we want to ignore as they are
    // handled in handleZoom()
  ...
  private void handleZoom(final ZoomEvent pEvent) {
    ...
    if (pEvent.getEventType() == ZoomEvent.ZOOM_STARTED
        && properties.activateGesture(GraphInputGesture.ZOOM, pEvent, this)) {
      pEvent.consume();
  ...
  /**
   * Starts panning. Should be called on mouse-pressed or when a drag event occurs without a pressed
   * event having been registered. This can happen if e.g. a context menu closes and consumes the
   * pressed event.
   *
