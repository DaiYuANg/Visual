...
public class GeometryUtils {
   ...
   *
   * <p>Only works for connectors that are attached to nodes.
   *
   * <p>
   *
   ...
   * @return the x and y coordinates of the connector, or {@code null} if the connector isn't
   *     attached to a node
   */
  public static Point2D getConnectorPosition(
      final GConnector connector, final SkinLookup skinLookup) {
   ...
   */
  public static @NotNull Point2D getCursorPosition(
   ...
   */
  public static @NotNull List<Point2D> getJointPositions(
      final @NotNull List<GJointSkin> jointSkins) {
   ...
   *
   * <p>Uses the JavaFX properties of the skins, not the model values. Is therefore always
   * up-to-date, even during a drag gesture where the model is not necessarily updated.
   *
   * <p>
   *
   ...
   */
  public static void fillJointPositions(
      final @NotNull GConnection connection, final SkinLookup skinLookup, final Point2D[] pTarget) {
   ...
   */
  public static @NotNull List<Point2D> getJointPositions(
      final @NotNull GConnection connection, final SkinLookup skinLookup) {
    return connection.getJoints().stream()
        .map(joint -> getJointPosition(joint, skinLookup))
        .collect(
            Collectors.toCollection(() -> new ObjectArrayList<>(connection.getJoints().size())));
  }
   ...
   */
  public static @NotNull Point2D getJointPosition(
      final GJoint joint, final @NotNull SkinLookup skinLookup) {
   ...
   *
   * <p>Lines drawn off-pixel look blurry. They should therefore have integer x and y values.
   *
   ...
   *
   * <p>Also returns true if the given position is equal to either of the values.
   *
  ...
  public static boolean checkInRange(
      final double firstValue, final double secondValue, final double position) {
  ...
  public static boolean checkIntersection(
      final @NotNull Point2D a, final Point2D b, final Point2D c, final Point2D d) {

    if (!(c.getX() > a.getX() && c.getX() < b.getX())
        && !(c.getX() > b.getX() && c.getX() < a.getX())) {
      return false;
    }
   ...
   */
  public static double roundToGridSpacing(
      final GraphEditorProperties pProperties, final double pValue) {
