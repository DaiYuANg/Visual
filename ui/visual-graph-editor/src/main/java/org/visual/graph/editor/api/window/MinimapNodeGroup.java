 ...
 *
 * <p>This is responsible for drawing mini versions of all nodes in a {@link GModel}. This group of
 * mini-nodes is then displayed inside the {@link GraphEditorMinimap}.
 */
class MinimapNodeGroup extends Parent {
  ...
  private final InvalidationListener checkSelectionWeakListener =
      new WeakInvalidationListener(checkSelectionListener);

  private SelectionManager selectionManager;

  /**
   * -- SETTER -- Sets the model whose nodes will be drawn in the minimap.
   *
   * @param pModel the {@link GModel} whose nodes are to be drawn
   */
  @Setter private GModel model;
  ...
  /**
   * -- SETTER -- Set a filter to only draw the desired connections onto the minimap
   *
   * @param pConnectionFilter connection filter {@link Predicate}
   */
  @Setter private Predicate<GConnection> connectionFilter = c -> true;
  ...
  private final StyleableObjectProperty<Color> connectionColor =
      new StyleableObjectProperty<>(Color.GRAY) {
   ...
   *
   * <p>This will be used to show what nodes are currently selected.
   *
   * <p>
   *
  ...
  /**
   * @return {@link ObjectProperty} controlling the {@link Color} to paint the connections onto the
   *     minimap
   */
  ...
  /**
   * @param pScaleFactor the ratio between the size of the content and the size of the minimap
   *     (between 0 and 1)
   */
  ...
  public void draw() {
    ...
    if (model != null) {
      for (int i = 0; i < model.getNodes().size(); i++) {
        ...
        final Node minimapNode =
            minimapRenderer == null ? null : minimapRenderer.createMinimapNode(node);
        if (minimapNode != null) {
  ...
  private <N extends Node> void resizeRelocate(
      final GNode node, final Node rendered, final IMinimapRenderer<N> pRenderer) {
  ...
  private static class StyleableProperties {
    ...
    static final CssMetaData<MinimapNodeGroup, Color> CONNECTION_COLOR =
        new CssMetaData<>("-connection-color", StyleConverter.getColorConverter(), Color.GRAY) {

          @Override
          public boolean isSettable(final @NotNull MinimapNodeGroup node) {
            return !node.connectionColor.isBound();
          }

          @Contract(pure = true)
          @Override
          public StyleableProperty<Color> getStyleableProperty(@NotNull MinimapNodeGroup node) {
