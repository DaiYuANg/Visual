 ...
 *
 * <p>This is a subclass of {@link StackPane} and will lay out its children accordingly. The size of
 * the box should be set via {@code resize(width, height)}, and will not be affected by parent
 * layout.
 */
@Getter
@Setter
...
public class DraggableBox extends StackPane {
  ...
  /**
   * -- SETTER -- Sets the editor properties object that the drag logic should respect.
   *
   * <p>This method is called by the framework. Custom skins should <b>not</b> call it. Editor
   * properties should instead be set via the graph editor instance.
   *
   * @param pEditorProperties the {@link GraphEditorProperties} instance for the graph editor
   */
  @Setter private GraphEditorProperties editorProperties;

  /**
   * -- GETTER -- Gets the set of x values that the box will align to when dragged close enough.
   *
   * <p>This mechanism will be active if the list is not and not empty. If both this mechanism and
   * snap-to-grid are active, snap-to-grid will take priority. -- SETTER -- Sets the set of x values
   * that the box will align to when dragged close enough.
   *
   * <p>This mechanism will be active if the list is not and not empty. If both this mechanism and
   ...
   * @param pAlignmentTargetsX a list of x values that the box will align to when dragged, or {@code
   *     null}
   */
  // Note that ResizableBox subclass currently pays no attention to alignment targets!
  @Setter @Getter private double[] alignmentTargetsX;

  /**
   * -- GETTER -- Gets the set of y values that the box will align to when dragged close enough.
   *
   * <p>This mechanism will be active if the list is not and not empty. If both this mechanism and
   * snap-to-grid are active, snap-to-grid will take priority. -- SETTER -- Sets the set of y values
   * that the box will align to when dragged close enough.
   *
   * <p>This mechanism will be active if the list is not and not empty. If both this mechanism and
   * snap-to-grid are active, snap-to-grid will take priority.
   ...
   * @param pAlignmentTargetsY a list of y values that the box will align to when dragged, or {@code
   *     null}
   */
  @Setter @Getter private double[] alignmentTargetsY;

  /**
   * -- GETTER -- Gets the alignment threshold value.
   *
   * <p>If the distance between the box and an alignment value is less than or equal to this
   * threshold, the box will move to the alignment value. -- SETTER -- Sets the alignment threshold
   * value.
   *
   * <p>If the distance between the box and an alignment target is lower than this threshold, the
   * box will move to the alignment target.
   *
   * @return the alignment threshold value
   * @param pAlignmentThreshold the alignment threshold value
   */
  @Setter @Getter private double alignmentThreshold = DEFAULT_ALIGNMENT_THRESHOLD;

  /**
   * -- GETTER -- Gets the offset that the box will snap to when a drag finishes and snap-to-grid is
   * active.
   *
   * <p>-- SETTER -- Sets the offset that the box will snap to when a drag finishes and snap-to-grid
   * is active.
   *
   * <p>The offset is taken from the top-left corner and is <b>(0, 0)</b> by default. If the default
   * value is used, the top-left corner of the box will snap exactly onto a grid line.
   *
   ...
   */
  @Setter @Getter private Point2D snapToGridOffset = Point2D.ZERO;
  ...
  /**
   * Set a dependent {@link DraggableBox} that will be moved (on the X axis) when this box is moved
   * on the X-Axis.
   *
   * @param pDependencyX dependent {@link DraggableBox}
   */
  ...
  /**
   * Set a dependent {@link DraggableBox} that will be moved (on the Y axis) when this box is moved
   * on the Y-Axis.
   *
   * @param pDependencyY dependent {@link DraggableBox}
   */
   ...
   *
   * @return {@code true} if the mouse is near the edge of the rectangle so that a resize would
   *     occur
   */
   ...
   *
   * @param pEvent a {@link MouseEvent}
   */
   ...
   *
   * @param pX the container-x position of the click event
   * @param pY the container-y position of the click event
   */
  ...
  /**
   * will be called when the position of this draggable box has been moved by the user
   *
   * @since 16.01.2019
   */
   ...
   *
   * @param pX the cursor x position relative to the container
   * @param pY the cursor y position relative to the container
   */
  ...
  protected double getNorthBoundValue() {
    return editorProperties != null
        ? editorProperties.getNorthBoundValue()
        : GraphEditorProperties.DEFAULT_BOUND_VALUE;
  }
   ...
   *
   * @param pX the cursor x position
   */
  private void handleDragX(final double pX) {
    ...
    if (isSnapToGrid()) {
      newLayoutX =
          roundToGridSpacing(newLayoutX - snapToGridOffset.getX()) + snapToGridOffset.getX();
    } else {
      // Even if snap-to-grid is off, we use Math.round to ensure drawing 'on-pixel' when
      // zoomed in
      // past 100%.
   ...
   *
   * @param pY the cursor y position
   */
  private void handleDragY(final double pY) {
    ...
    if (isSnapToGrid()) {
      newLayoutY =
          roundToGridSpacing(newLayoutY - snapToGridOffset.getY()) + snapToGridOffset.getY();
    } else {
      // Even if snap-to-grid is off, we use Math.round to ensure drawing 'on-pixel' when
      // zoomed in
      // past 100%.
  ...
  /**
   * Gets the closest ancestor (e.g. parent, grandparent) to a node that is a subclass of {@link
   * Region}.
   *
   * @param node a JavaFX {@link Node}
   * @return the node's closest ancestor that is a subclass of {@link Region}, or {@code null} if
   *     none exists
   */
  ...
  /**
   * Aligns the given position to the first alignment value that is closer than the alignment
   * threshold.
   *
   * <p>Returns the original position if no alignment values are nearby.
   *
  ...
  private double align(final double position, final double[] alignmentValues) {
    return Arrays.stream(alignmentValues)
