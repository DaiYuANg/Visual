 ...
 *
 * <p>See {@link DraggableBox} for more information.
 */
public class ResizableBox extends DraggableBox {
  ...
  protected void handleMouseDragged(final @NotNull MouseEvent pEvent) {
    if (lastMouseRegion == null
        || !(getParent() instanceof Region)
        || !pEvent.isPrimaryButtonDown()
        || !isEditable()) {
      setCursor(null);
      return;
    }
   ...
   *
   * @param pEvent the latest {@link MouseEvent} for the mouse entering or moving inside the
   *     rectangle
   */
   ...
   *
   * <p>This should occur for example when the cursor is near the border of the rectangle, to
  ...
  private void updateCursor(final RectangleMouseRegion mouseRegion) {
    switch (mouseRegion) {
      ...
      case INSIDE:
      case OUTSIDE:
        // Set to null instead of Cursor.DEFAULT so it doesn't overwrite cursor settings of
        // parent.
        setCursor(null);
        break;
    }
   ...
   *
   * <p>Used during mouse hover and drag events on a {@link ResizableBox}.
   */
