package org.visual.graph.editor.core.skins;
...
import org.visual.graph.editor.core.skins.defaults.*;
...
public class GraphEditorSkinManager implements SkinManager {
  ...
  /**
   * Creates a new skin manager instance. Only one instance should exist per {@link
  ...
  public void setConnectorSkinFactory(
      final Callback<GConnector, GConnectorSkin> pConnectorSkinFactory) {
    mConnectorSkinFactory = pConnectorSkinFactory;
  }

  @Override
  public void setConnectionSkinFactory(
      final Callback<GConnection, GConnectionSkin> pConnectionSkinFactory) {
  ...
  private @NotNull GConnectorSkin createConnectorSkin(final GConnector pConnector) {
    GConnectorSkin skin =
        mConnectorSkinFactory == null ? null : mConnectorSkinFactory.call(pConnector);
    if (skin == null) {
  ...
  private @NotNull GConnectionSkin createConnectionSkin(final GConnection pConnection) {
    GConnectionSkin skin =
        mConnectionSkinFactory == null ? null : mConnectionSkinFactory.call(pConnection);
    if (skin == null) {
  ...
  private void positionMoved(final GSkin<?> pMovedSkin) {
    ...
    if (pMovedSkin instanceof GNodeSkin gns) {
      ...
      gns.getItem().getConnectors().stream()
          .map(GConnector::getConnections)
          .forEach(layouter::redraw);
      return;
    }
