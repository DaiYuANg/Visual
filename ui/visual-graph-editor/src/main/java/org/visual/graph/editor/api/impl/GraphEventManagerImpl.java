package org.visual.graph.editor.api.impl;
...
public class GraphEventManagerImpl implements GraphEventManager {
  ...
  @Override
  public boolean activateGesture(
  ...
  private boolean canActivate(final GraphInputGesture pGesture, final Event pEvent) {
    ...
    if (current == pGesture) {
      ...
      final boolean isTouch =
          pEvent instanceof TouchEvent
              || pEvent instanceof MouseEvent me && me.isSynthesized()
              || pEvent instanceof ScrollEvent se && se.getTouchCount() > 0;
      if (!isTouch) {
        return switch (pGesture) {
          case PAN ->
              pEvent instanceof ScrollEvent se && !se.isControlDown()
                  || pEvent instanceof MouseEvent me && me.isSecondaryButtonDown();
          case ZOOM -> pEvent instanceof ScrollEvent se && se.isControlDown();
          case SELECT, CONNECT, MOVE, RESIZE ->
              pEvent instanceof MouseEvent me && me.isPrimaryButtonDown();
        };
      } else {
        return switch (pGesture) {
