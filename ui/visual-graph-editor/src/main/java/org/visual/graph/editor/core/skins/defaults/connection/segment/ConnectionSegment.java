 ...
 */
package org.visual.graph.editor.core.skins.defaults.connection.segment;
 ...
 *
 * <p>A segment is a horizontal or vertical line either (a) between two joints, or (b) between a
 * connector and a joint.
 */
public abstract class ConnectionSegment {
  ...
  /**
   * -- GETTER -- Returns the this connection segment is made of.
   *
   * @return the path elements this segment is made of
   */
  @Getter private final List<PathElement> pathElements = new ArrayList<>();

  /**
   * -- GETTER -- Returns the start of this connection segment.
   *
   * @return the start point of this connection segment
   */
  @Getter private final Point2D start;

  /**
   * -- GETTER -- Returns the end of this connection segment.
   *
   * @return the end point of this connection segment
   */
  @Getter private final Point2D end;
  ...
  public ConnectionSegment(
      final @NotNull Point2D start, final @NotNull Point2D end, final double[] intersections) {
    this.start = start;
    this.end = end;
   ...
   *
   * <p>Draws a <b>detour</b> for each of the other connections that vertically intersect this
   * segment, provided there is enough room to do so. Does not draw a detour at points where a
   * connection intersects itself.
   */
  public void draw() {
   ...
   *
   * @param intersection the intersection position to take into account when drawing this element
   * @param lastIntersection the previous intersection position to take into account when drawing
   *     this element
   */
  protected abstract void drawBetweenIntersections(
      final double intersection, final double lastIntersection);
