 ...
 *
 * <p>This extends {@link PanningWindowMinimap}, additionally displaying a small rectangle for each
 * of the nodes in the currently edited model.
 */
public class GraphEditorMinimap extends PanningWindowMinimap {

  // Until the content is set, we don't know the aspect ratio of the minimap. Use this value until
  // then.
  private static final double INITIAL_ASPECT_RATIO = 0.75;

  // Minimap height is not specified here, the minimap's aspect ratio is fixed by the aspect ratio
  // of the content.
  ...
  protected double computePrefHeight(double width) {
    ...
    final double contentRatio =
        getContent() == null
            ...
            : getContent().getHeight() / getContent().getWidth();
  ...
  /**
   * Set a filter {@link Predicate} to only draw the desired connections onto the minimap. The
   * default is to show all connections.
   *
