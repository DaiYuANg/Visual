package org.visual.model.text.editor.undo.impl;
...
public class FixedSizeChangeQueue<C> implements ChangeQueue<C> {
    ...
    public C peekNext() {
        if(currentPosition < size) {
            return fetch(currentPosition).change();
        } else {
    ...
    public C peekPrev() {
        if(currentPosition > 0) {
            return fetch(currentPosition - 1).change();
        } else {
            throw new NoSuchElementException();
        }
    }
    ...
    @Override
    @SafeVarargs
    public final void push(C @NotNull ... changes) {
        ...
        if(currentPosition > capacity) {
            ...
            size = capacity;
            zeroPositionRevision = lastOverwrittenChange.revision();
        } else {
            size = currentPosition;
        }
    }
    ...
    private long fetchRevisionForPosition(int position) {
        if(position == 0) {
            ...
            return fetch(position - 1).revision();
        }
    }
