...
public final class Foundation {
    ...
    public static void init() {
        /* fake method to init de.jangassen.foundation */
    }
    ...
    public static ID invokeVarArg(final ID id, final Pointer selector, Object... args) {
        // c functions and objc methods have at least 1 fixed argument, we therefore need to separate out the first
        // argument
    ...
    public static ID safeInvoke(final ID id, final String stringSelector, Object... args) {
        ...
        if (!id.equals(ID.NIL) && !invoke(id, "respondsToSelector:", selector).booleanValue()) {
            throw new RuntimeException(String.format(
                    "Missing selector %s for %s", stringSelector, toStringViaUTF8(invoke(id, "description"))));
        }
    ...
    public static boolean addIvar(ID cls, String name, String types) {
        return myFoundationLibrary.class_addIvar(
                cls, name, Native.LONG_SIZE, ((Double) Math.log(Native.LONG_SIZE)).intValue(), types);
    }
    ...
    public static @Nullable String toStringViaUTF8(ID cfString) {
        ...
        byte ok = myFoundationLibrary.CFStringGetCString(
    ...
    public static long getEncodingCode(String encodingName) {
        if (encodingName == null || encodingName.trim().isEmpty()) return -1;
    ...
    public static void executeOnMainThread(
            final boolean withAutoreleasePool, final boolean waitUntilDone, final Runnable runnable) {
