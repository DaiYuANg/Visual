...
public interface FoundationLibrary extends Library {
    ...
    ID CFStringCreateWithBytes(
            Pointer allocator, byte[] bytes, int byteCount, int encoding, byte isExternalRepresentation);
    ...
    double objc_msgSend_fpret(
            ID receiver, Pointer selector, Object... args); // the same as objc_msgSend but returns double (32-bit only)
    ...
    boolean class_addIvar(ID cls, String name, long size, int alignment, String types);

    Pointer class_getInstanceVariable(ID cls, String name);

    boolean object_setIvar(ID instance, Pointer ivar, ID value);

    ID object_getIvar(ID instance, Pointer ivar);
