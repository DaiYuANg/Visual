 ...
 */
package org.visual.model.ui.inspector.remote;
...
import org.visual.model.ui.inspector.*;
import org.visual.model.ui.inspector.details.DetailPaneType;
import org.visual.model.ui.inspector.event.FXConnectorEvent;
...
class RemoteConnectorImpl extends UnicastRemoteObject implements RemoteConnector, FXConnector {
    ...
    private void loadAgent(final VirtualMachine machine, final File agentFile) {
        try {
            ...
            log.info("Loading agent for:" + machine + " ID:" + machine.id() + " on port:" + port + " took:" + (System.currentTimeMillis() - start) + "ms using agent defined in " + agentFile.getAbsolutePath());
            vmInfo.put(port, machine.id());
            machine.loadAgent(agentFile.getAbsolutePath(), port + ":" + this.port + ":" + machine.id() + ":" + log.isDebugEnabled());
            machine.detach();
        } catch (final Exception e) {
            log.error(e.getMessage(), e);
        }
