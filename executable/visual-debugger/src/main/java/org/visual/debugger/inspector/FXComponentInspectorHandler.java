...
public class FXComponentInspectorHandler {
    ...
    private void inspectAllWindows() {
        Window.getWindows()
                .stream()
                .filter(window -> window != popup && window != highlightPopup && !handledWindows.contains(window))
                .forEach(window -> {
    ...
    private void inspectNode(@NotNull Node node) {
        node.addEventFilter(MouseEvent.MOUSE_MOVED, event -> handle(event, null));
    }


    private void handle(@NotNull MouseEvent event, Scene scene) {
        ...
        if (!(event.getTarget() instanceof Node componentUnderMouse)) {
            return;
        }
    ...
    private void highlightComponent(@NotNull Node component, @NotNull Scene scene) {
