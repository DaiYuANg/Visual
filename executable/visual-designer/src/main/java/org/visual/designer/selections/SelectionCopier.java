/* (C)2024*/
package org.visual.designer.selections;
...
import org.visual.graph.editor.core.connections.ConnectionCopier;
import org.visual.graph.editor.model.*;

/**
 * Manages cut, copy and paste actions on the current selection.
 *
 * <p>The rules for what is copied are as follows:
 *
 * <ol>
 *   <li>All selected nodes and their connectors (containment references) are copied.
 *   <li>If a connection's source and target nodes are <b>both</b> copied, the connection and its
 *       joints are also copied.
 * </ol>
 */
public class SelectionCopier {
  ...
  public void copy() {
    ...
    // Don't iterate directly over selectionTracker.getSelectedNodes() because that will not
    // preserve ordering.
   ...
   *
   * <p>After the paste operation, the newly-pasted elements will be selected.
   *
   ...
   */
  private void preparePastedElements(
      final List<GNode> pastedNodes, final List<GConnection> pastedConnections) {
   ...
   *
   * <p>Corrects the x and y positions accordingly if they will be outside the bounds.
   *
   * @param pastedNodes the nodes that are going to be pasted
   * @param pastedConnections the connections that are going to be pasted
   */
   ...
   * @param consumer a consumer to allow custom commands to be appended to the paste command
   */
  private void addPastedElements(
      final List<GNode> pastedNodes,
      final List<GConnection> pastedConnections,
  ...
  /**
   * Determines the offset by which the new nodes and joints should be positioned relative to the
   * nodes and joints they were copied from.
   *
   * <p>The aim here is to paste the new nodes and joints into a <b>visible</b> area on the screen,
   * even if the user has panned around in the graph editor container since the copy-action took
   * place.
   *
